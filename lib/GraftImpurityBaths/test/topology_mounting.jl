...
using GraftFoundation: FermionParity, TreeTopology, blocks, dim, is_t3ns,
    nodeid, nodeindex, nnodes, sectors
using GraftNetworks: TTNO
using GraftSymbolic: OpSum, fermion_ops_z2
using GraftTTNOBuild: ttno_from_opsum
using GraftImpurityFoundations: FermionParitySector, FlavorLayout,
    ParticleNumberSector, Partition, FermionSiteOperators, fermion_sector,
    local_annihilator, local_creator, local_number, physical_site
import GraftImpurityInteractions
using GraftImpurityBaths: AndersonBath, BathOrbitals, BosonBath, DiscreteBath,
    FTPS, T3NS, impurity_topology, mount_bath
...
function _is_descendant(topology::TreeTopology, node::Symbol, ancestor::Symbol)
    index = nodeindex(topology, node)
    ancestor_index = nodeindex(topology, ancestor)
    while index != 0
...
@testset "M5 ownership-preserving topology and mounting" begin
    ...
    @test is_t3ns(t3ns; physical)
    @test count(node -> !(nodeid(t3ns, node) in physical),
                1:nnodes(t3ns)) == 1
    @test nodeid(t3ns, t3ns.root) == :imp_c
    ...
    @test [nodeid(ftps, index) for index in 1:3] == spine
    @test !is_t3ns(ftps; physical)
    for (site, owner) in zip(expected_bath_sites,
                              bath.orbitals.associated_flavors)
    ...
    @test is_t3ns(zero_tooth_t3ns;
                  physical=Symbol[:imp_a, :imp_b, :imp_c,
                                  :bath_a_1, :bath_a_2, :bath_a_3, :bath_a_4])
    @test count(node -> nodeid(zero_tooth_t3ns, node) == :t3ns_junction_1,
                1:nnodes(zero_tooth_t3ns)) == 0
    ...
    @test is_t3ns(minimal_t3ns; physical=minimal_physical)
    @test count(node -> !(nodeid(minimal_t3ns, node) in minimal_physical),
                1:nnodes(minimal_t3ns)) == 0
    ...
    @test length(sectors(u1_local_ops.P)) == 3
    u1_mounted = mount_bath(
        TreeTopology(:cluster, Pair{Symbol,Symbol}[]), shared_bath,
    )
    u1_physical = _physical_dict(u1_mounted)
    @test length(sectors(u1_physical[:cluster])) == 3
    @test length(sectors(u1_physical[first(u1_mounted.sites)])) == 2
