function _topology_edges(topology::TreeTopology)
    return Pair{Symbol,Symbol}[
        nodeid(topology, topology.parent[node]) => nodeid(topology, node)
        for node in 2:nnodes(topology)
    ]
end
...
function _topology_with_mounted_sites(topology::TreeTopology, bath::DiscreteBath,
    ...
    return TreeTopology(nodeid(topology, topology.root), edges), :extended
end
