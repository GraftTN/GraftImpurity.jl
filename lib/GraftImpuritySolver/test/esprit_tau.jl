...
@testset "imaginary-time ESPRIT bath-fitting kernel" begin
    ...
    @test kernel.reduction isa GraftSpectral.AllComponents
    ...
    declared_diagonal = real_pole_bath_fit(
        ...
        ESPRITTauKernel(
            n_poles=2, pole_tolerance=1e-8,
            projection_tolerance=1e-10, fit_tolerance=1e-7,
            reduction=GraftSpectral.DeclaredDiagonal(
    ...
    @test declared_diagonal.trace.fits[1].node_estimate.common.reduction.policy isa
          GraftSpectral.DeclaredDiagonal
    ...
    @test lowered.operator isa TTNO
