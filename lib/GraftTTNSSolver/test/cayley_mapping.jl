...
function _test_cayley_topology_extension(mounted::CayleyAndersonBath)
    ...
    source_count = nnodes(source)
    source_root = nodeid(source, source.root)
    @test nodeid(target, target.root) == source_root
    ...
    for source_node in 1:source_count
        ...
        target_node = nodeindex(
            target, nodeid(source, source_node),
        )
        ...
        if source.parent[source_node] == 0
            ...
            @test nodeid(target, target.parent[target_node]) ==
                  nodeid(source, source.parent[source_node])
        end
    end
    for impurity_site in layout_sites(bath_layout(mounted.mapping.mapped))
        impurity_node = nodeindex(target, impurity_site)
        @test nodeid(target, target.parent[impurity_node]) == source_root
    end
    return nothing
end
...
@testset "M5 scalar Cayley bath mapping" begin
    ...
    @test nodeid(result.mapped.topology,
    ...
    for z in (0.7im, -0.25 + 0.8im)
        @test _cayley_delta(
            GraftImpurityBaths._cayley_coupling_matrix(bath),
    ...
    @test all(nchildren(result.mapped.topology,
                        nodeindex(result.mapped.topology, edge.parent)) <= 2
    ...
    @test nodeid(action_split_mounted.topology,
    ...
    @test nodeid(dark_mounted.topology,
    ...
    @test dim(getproperty(shared_mounted.phys, :cluster)) == 4
...
@testset "M5 block Cayley bath mapping" begin
    ...
    descendant_topology = TreeTopology(
    ...
    canonical_onebody = vcat(
        hcat(impurity_onebody, GraftImpurityBaths._cayley_coupling_matrix(bath)),
        hcat(adjoint(GraftImpurityBaths._cayley_coupling_matrix(bath)),
    ...
    @test nodeid(split_mounted.topology,
    ...
    for z in (0.5im, 0.3 + 0.9im)
        @test _cayley_delta(
            GraftImpurityBaths._cayley_coupling_matrix(reduced),
