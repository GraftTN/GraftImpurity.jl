...
@testset "experimental scalar LorentzianPSD" begin
    ...
    objective, gradient! = GraftImpurityPoleFits._lorentzian_objective(
...
@testset "experimental matrix LorentzianPSD" begin
    ...
    width_floor = 0.01
    parameters = GraftImpurityPoleFits._matrix_lorentzian_pack(
        gradient_centers, gradient_widths, factors, width_floor)
    kernel = GraftImpurityPoleFits._lorentzian_kernel(
        frequencies, centers, widths)
    gradient_target = GraftImpurityPoleFits._matrix_lorentzian_evaluate(
        kernel, residues)
    scale = sqrt(sum(norm(value)^2 for value in gradient_target))
    objective, gradient! = GraftImpurityPoleFits._matrix_lorentzian_objective(
    ...
    _, hermitian_samples, positive_samples, _ =
        GraftImpurityPoleFits._lorentzian_matrix_samples(
            nonseparable_samples, frequencies)
    fixed_kernel = hcat(kernel_a, kernel_b)
    positive_initialization = GraftImpurityPoleFits._matrix_lorentzian_initial_residues(
        fixed_kernel, positive_samples, 0.0)
    raw_initialization = GraftImpurityPoleFits._matrix_lorentzian_initial_residues(
    ...
    activated_residue = only(GraftImpurityPoleFits._matrix_lorentzian_initial_residues(
