...
@testset "PES pole fitting" begin
    ...
    @test GraftImpurityPoleFits._pes_samples_are_exactly_diagonal(diagonal_values)
    ...
    @test !GraftImpurityPoleFits._pes_samples_are_exactly_diagonal(
    ...
    trial_weights, residual, _ = GraftImpurityPoleFits._pes_fit_weights(
        trial_poles, boson_frequencies, trial_samples, :least_squares, :boson)
    residual_norm = GraftImpurityPoleFits._pes_residual_norm(residual)
    analytic = GraftImpurityPoleFits._pes_pole_gradient(
    ...
    finite_difference = map(eachindex(trial_poles)) do k
        ...
        _, plus_residual, _ = GraftImpurityPoleFits._pes_fit_weights(
            plus, boson_frequencies, trial_samples, :least_squares, :boson)
        _, minus_residual, _ = GraftImpurityPoleFits._pes_fit_weights(
            minus, boson_frequencies, trial_samples, :least_squares, :boson)
        (GraftImpurityPoleFits._pes_residual_norm(plus_residual) -
         GraftImpurityPoleFits._pes_residual_norm(minus_residual)) / (2step)
    end
