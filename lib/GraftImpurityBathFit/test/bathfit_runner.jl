...
function _health_runner_samples(input::BathFitInput)
    return copy(GraftImpurityBathFit._bathfit_realimag_sample_vector(input))
end
...
function _health_runner_covariance(input::BathFitInput; variance=1e-6)
    dimension = length(GraftImpurityBathFit._bathfit_realimag_sample_vector(input))
...
@testset "opt-in perturb-and-refit bath diagnostics runner" begin
    ...
    for behavior in (:nonconverged_status, :nonconverged_diagnostics)
        candidate = GraftImpurityBathFit._bathfit_run_candidate(
    ...
    first_replica = GraftImpurityBathFit._bathfit_input_with_realimag(
        fixture.training,
        GraftImpurityBathFit._bathfit_realimag_sample_vector(fixture.training) .+ 1e-4,
    )
    second_replica = GraftImpurityBathFit._bathfit_input_with_realimag(
        fixture.training,
        GraftImpurityBathFit._bathfit_realimag_sample_vector(fixture.training) .- 2e-4,
    )
    empty!(_HEALTH_RUNNER_CALLS)
...
@testset "complex matrix covariance embedding keeps every entry" begin
    ...
    embedded = GraftImpurityBathFit._bathfit_realimag_sample_vector(matrix_input)
    rebuilt = GraftImpurityBathFit._bathfit_input_with_realimag(matrix_input, embedded)
    @test GraftImpurityBathFit._bathfit_realimag_sample_vector(rebuilt) == embedded
