...
@testset "real-pole fitting kernels" begin
    ...
    @test GraftImpurityBathFit._interval_bin_grid(forward_forced) ==
          GraftImpurityBathFit._interval_bin_grid(reverse_forced)
    close_forced = SpectralInterval(-1.0, 1.0, 3; forced_poles=[0.02, -0.02])
    _, close_poles = GraftImpurityBathFit._interval_bin_grid(close_forced)
