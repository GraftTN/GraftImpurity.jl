...
function _esprit_tau_nodes(samples::Vector{Matrix{ComplexF64}},
    ...
    estimator = ESPRIT(
        rank=StrictRank(kernel.n_poles, NumericalRank()),
        reduction=kernel.reduction,
        hankel_rows=row_count,
    )
    estimate = estimate_nodes(
        estimator, UniformSequence(taus, samples),
    )
    outcome = estimate.outcome
    if outcome isa AbstractNodeFailure
        backend = estimate.backend
        common = estimate.common
        message = if backend isa ESPRITDiagnostics &&
        ...
        elseif outcome isa NodeEstimationFailure &&
    ...
    elseif outcome isa ZeroSequence
        throw(ArgumentError(
            "ESPRITTauKernel zero sequence must use the zero-fit path",
        ))
    elseif !(outcome isa IdentifiedNodes)
        throw(ArgumentError(
    ...
    backend = estimate.backend
    backend isa ESPRITDiagnostics || error(
