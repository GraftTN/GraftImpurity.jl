...
@testset "bath-fit spectral details" begin
    ...
    metric = GraftImpurity._bathhealth_metric(
        covariance, :validation, validation,
    )
    details = GraftImpurity._bathhealth_spectral_details(
    ...
    uncalibrated = GraftImpurity._bathhealth_metric(
        nothing, :validation, validation,
    )
    uncalibrated_details = GraftImpurity._bathhealth_spectral_details(
    ...
    indefinite_details = GraftImpurity._bathhealth_spectral_details(
    ...
    flux = GraftImpurity._bathhealth_boundary_flux(
...
@testset "bath-fit pole spacing is block local" begin
    ...
    @test GraftImpurity._bathhealth_minimum_pole_spacing(expansion) == 2.0
    ...
    @test isnothing(
        GraftImpurity._bathhealth_minimum_pole_spacing(no_pairs),
    )
end
...
@testset "bath-fit transport uses Simpson energy integration" begin
    ...
    left = only(GraftImpurity._bathhealth_measures(
        _pure_health_expansion(fixture, [-1.0], [1.0]),
    ))
    right = only(GraftImpurity._bathhealth_measures(
        _pure_health_expansion(fixture, [1.0], [1.0]),
    ))
    state = reshape(ComplexF64[1], 1, 1)
    endpoint_left = GraftImpurity._bathhealth_fisher_resolution(
        validation, :block, -1.0, state, metric,
    )
    midpoint = GraftImpurity._bathhealth_fisher_resolution(
        validation, :block, 0.0, state, metric,
    )
    endpoint_right = GraftImpurity._bathhealth_fisher_resolution(
        validation, :block, 1.0, state, metric,
    )
    expected = 2 / 6 * (
        inv(endpoint_left) + 4inv(midpoint) + inv(endpoint_right)
    )
    @test GraftImpurity._bathhealth_transport(
